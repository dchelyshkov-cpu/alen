/* ALEN 10.0 — Current release runtime */
...
(() => {
  ...
  function focusAfterClose(target){
    if (!target || !target.isConnected || target.hidden || target.disabled) return;
  ...
  function setupCallbackModal(){
    ...
    function closeModal({restoreFocus = true} = {}){
      ...
      resetView();
      if (restoreFocus) focusAfterClose(returnTarget);
    }
    ...
    modal._closeForSharedForms = () => closeModal({restoreFocus:false});
  }
...
const initCookieConsent = () => {
  ...
  const showSettings = (source) => {
    settingsReturnTarget = source || document.activeElement;
    setSettingsValues(readConsent() || defaults);
    hideBanner();
    settings.removeAttribute("hidden");
    backdrop.removeAttribute("hidden");
    document.body.classList.add("cookie-settings-open");
    window.requestAnimationFrame(() => settings.querySelector(".cookie-settings-close")?.focus());
  };
  const focusAfterClose = (target) => {
    if(!target || !target.isConnected || target.hidden || target.disabled) return;
    window.requestAnimationFrame(() => target.focus());
  };
  ...
  banner.querySelectorAll("[data-cookie-action]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      if(action === "settings") showSettings(button);
    });
  });
  ...
  trigger.addEventListener("click", () => showSettings(trigger));
